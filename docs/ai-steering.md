...
- **Favicon**: `web/stock-market.png` set as the site favicon via `<link rel="icon" type="image/png">` in `index.html`
- **Masthead cleanup**: removed "Vol. 1" and timezone Edition labels from masthead topline; removed duplicate published date (kept only bottomline "Published …" span); changed stamp from "Printed for the next session" → "Published for the next trading session"; footer simplified from "{universe} Evening Desk" → "Evening Desk"
- **Compact table layout for Trending Tickers**: replaced card grid with a `<table>` — columns: #, Ticker (with company name stacked below in grey), Price, 1-Day, 3-Day, Vol Ratio, Note. CSS class `.trending-table` / `.trending-table-wrap` (horizontally scrollable)
- **Compact table layout for Top Stock Signals**: replaced card grid with matching `<table>` — columns: Ticker (name stacked below), Price, Change, Status, Score, Rules. `renderSignalTable` / `renderSignalRow` replace `renderSignalCard`; rule pills in the Rules cell are single-line (`white-space: nowrap; overflow: hidden`)
- **Pill height in tables**: `.trending-table .pill, .trending-table .score-badge` overrides global `min-height: 44px` → `30px` so table rows stay compact
- **Nav and section reorder**: Summary nav link removed (section kept); nav order changed to Trending → Watchlists → Stocks → Options → Earnings; Earnings Watch section moved to bottom of page (after Options Signals)
