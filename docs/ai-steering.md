...
5:10 PM PT  EventBridge → AggregatorFunction
              ...
              "high priority" = weighted_score ≥ 35; "matched" = lower score
              ...
              fetches Yahoo Finance trending list → enriches with 3-day price/volume data + headlines → trendingTickers[]
                (trending fetch happens before highlights so top-10 symbols appear in Today's Highlights)
              merges high-priority screener symbols + trending symbols (deduped, capped 10) for news context
              Today's Highlights (5 bullets):
                1. "{matched} of {total} stocks matched at least one rule — X% breadth" (full universe)
                2. "Top conviction (score ≥40): N tickers — AA, BB, ..." (top 10 by weighted_score)
                3. "N tickers report earnings this week (notably: AA, BB) — watch for elevated implied volatility"
                4. "Yahoo trending from the past 3 days: AA, BB, ..." (top 10 from Yahoo Finance)
                5. "Options watching: AAAA260508P00089000, ..." (OCC-style contract labels)
...
- **Trigger Values section**: the `.reason-conditions` block has a header row with "TRIGGER VALUES" on the left and a **Bullish/Bearish sentiment tag** on the right (inferred from `ruleNames` — any rule name matching `/bearish/i` → Bearish, otherwise Bullish). Below the condition pills, matched rule names from `signal.ruleNames` are rendered as a second row of `.rule-tag` pills separated by a thin border.
...
- **Live price overlay**: on every analysis request (cache hit or miss), `_fetch_live_metrics` fetches the current price + intraday change% via `yf.fast_info` (no history, ~500 ms) and returns them as `liveMetrics` in the response — never persisted to S3; the frontend's `_applyLiveMetrics` updates the detail-page header with the current price so it always reflects today's data, not last night's EOD close
- **Recent news in trading brief**: on cache miss, `fetch_ticker_headlines` fetches up to 5 Yahoo Finance RSS headlines for the ticker; they are injected into the Gemini prompt as a "RECENT NEWS (past 24 hours)" block so the AI analysis can reference what actually drove the stock; news is NOT cached (the analysis text is cached, so regeneration on a new report date picks up fresh headlines)
...
- **Ticker Compliance theme fix** (`styles.css`): replaced all undefined CSS variables (`--color-ink-muted`, `--color-border`, `--color-paper-alt`, `--color-accent`, `--color-paper`, `--font-mono`, `--font-sans`) and Bootstrap-style hardcoded badge colors with the actual design tokens (`--ink`, `--paper`, `--accent`, `--muted`, `--neutral-*`). Removed `border-radius` from compliance badges and rule tags to match the global `border-radius: 0` theme rule. Win rate badges now use an ink-based system: filled black = high (≥65%), grey fill = mid (≥50%), outlined = low.
- **Earnings Watch section order fix** (`report-renderer.js`): moved Earnings Watch before Top Stock Signals in both the quick-nav and the HTML section order to match test expectations and logical calendar-first reading flow.
...
- **Earnings API key** in AWS Secrets Manager as `stock-analysis/earnings-api-key`; local dev uses `EARNINGS_API_KEY`. Workers use yfinance for dates, then call Earnings API only for the distinct dates yfinance returned. Daily Earnings API responses are cached in S3 at `raw/earnings-api/date=YYYY-MM-DD/calendar.json` to stay within the free-tier limit. **Cache TTL**: past dates are cached forever (already finalized); future/upcoming dates are automatically re-fetched if the S3 object is older than 7 days (`_EARNINGS_CACHE_TTL_DAYS`) so that companies' revised dates and timing (Before Open / After Close) are picked up weekly.
